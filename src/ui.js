...
var onRun = function(context) {
  var panel = NSAlert.alloc().init();
  // panel.setFrame_display(NSMakeRect(0, 0, 300, 300), true);
  panel.setMessageText('Sketch GraphQL');
   panel.setInformativeText('v0.1');
panel.addButtonWithTitle('Cancel');

panel.setIcon(NSImage.alloc().initWithContentsOfFile(
      context.plugin.urlForResourceNamed('logo.png').path()));
  var nibui = new NibUI(context,
      'UIBundle', 'MyNibUI',
      ['endpointField', 'queryField', 'submitButton']);

  nibui.attachTargetAndAction(nibui.submitButton, function() {
    log("Asd");
    context.document.showMessage('hieeee');
  });

  panel.setAccessoryView(nibui.view);
